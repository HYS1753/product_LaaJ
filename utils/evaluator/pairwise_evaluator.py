...
class PairwiseEvaluator:
    def __init__(self, llm_client):
        ...
        **상위 {k}개 기준으로 쿼리를 더 잘 만족하는 시스템**을 선택하세요.
        ...
        - PromotionValue (0~2): 동일/유사 relevance 후보 중,
          검증 가능한 할인·쿠폰·카드혜택·사은품이 더 우수한 상품이 상위에 배치되었는지
        ...
        [프로모션 반영 원칙]
        - 프로모션 혜택은 **보조 우선순위**입니다.
        - 먼저 Relevance와 ConstraintFit을 판단한 뒤,
          그 차이가 작거나 비슷할 때 PromotionValue로 우열을 가르세요.
        - 쿼리 핵심 조건이 맞지 않는 상품은 큰 할인 혜택이 있어도 우대하지 마세요.
        - 검증 가능한 수치/조건이 없는 홍보성 할인 문구는 PromotionValue 근거로 인정하지 마세요.
